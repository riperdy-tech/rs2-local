...
TOPICS = [
    ("Competitive Position & Moat",
     "{subj} competitive position, market share, moat durability, customer switching costs, "
     "captive installed-base fleet size (units, GW, or installed systems in the field), and long-term service agreements (CSAs/LTSAs) in {yr}"),
    ("Catalysts & Guidance",
     "{subj} latest quarterly/annual earnings results and explicit forward management guidance for {yr} 2026 2027 2028: "
     "For this specific company's business model (e.g. ARR/RPO/NRR for software; Fleet/TCE/orderbook for shipping; "
     "NIM/credit-losses for financials; Installed-base/CSAs for industrials; Production/Capex for energy), "
     "extract exact forward revenue guidance targets, segment EBITDA margins, contracted backlog/visibility, "
     "and planned capital expenditures (capex). "
     "Also explicitly identify any one-time non-operating gains/losses, M&A remeasurement gains, divestitures, or tax windfalls that distort trailing GAAP net income, and report normalized operating EBITDA and net income alongside reported figures. "
     "Tabulate all forward targets, operational metrics, and normalized earnings into a structured Operational Evidence Table with primary source citations."),
...
def ldr_overrides(tool):
    o = {
        ...
        # Exa MUST keep include_full_content=True (LDR's default). Unlike searxng/serper/tavily,
        # the Exa engine does NOT override _get_full_content -- it inherits the base default,
        # which only unpacks the `_full_result` Exa already returned and never scrapes. So the
        # API response is the ONLY source of page text on this engine: turning the flag off
        # silently reduces Exa to snippets while snippets_only=False still claims full pages.
        # Exa bills contents on top of search; that is the price of parity, not waste.
        "search.engine.web.exa.default_params.include_full_content": True,
    }
...
# Keyed providers, and the secret each one reads. All four are first-class LDR engines taking
...
LOW_TRUST_DOMAINS = ("support.trustwave.com", "pitchgrade.com", "ainvest.com", "koalagains.com",
                     ...
                     "pestel-analysis.com",
                     # 2026-08-29 batch — every entry read and verified before listing:
                     #   alert-invest.com     templated auto-generated stock pages, no authors,
                     #                        internally inconsistent metrics (P/E "0x", DCF $48
                     #                        against a $150 price on the same page)
                     #   stockoracle.com      self-describes as "an AI-aided stock intelligence
                     #                        web app powered by Piranha Profits"
                     #   brimindinvest.com    self-describes "BriMind AI" generating the scores
                     #                        and targets on every page
                     #   stockexpertai.com    page metadata literally says content generation
                     #                        "machine", "generated by model: gemini-2.0-flash"
                     #   kavout.com           AI-analysis platform ("AI Stock Picker",
                     #                        "InvestGPT") — model output, koalagains class
                     #   ems.eduwisland.edu.my  a Malaysian preschool-management portal whose
                     #                        subdomain served /market-insights/ stock articles —
                     #                        hijacked/abused, support.trustwave.com class
                     # Read and NOT listed: ziplaw.uk (real legal-careers publisher; its GOOG
                     # citation was a genuine business roundup), basisreport.com (self-described
                     # human analyst desk), technologychecker.io (bylined human author, hybrid),
                     # litefinance.org (named human editor; aggregated junk but not confirmed AI).
                     "alert-invest.com", "stockoracle.com", "brimindinvest.com",
                     "stockexpertai.com", "kavout.com", "ems.eduwisland.edu.my")

# The trustwave spam network is identified by its URL PATH, not its domain: the same templated
# stock articles appear under /expert-time/ and sibling slugs on a rotating set of hijacked or
# abused domains — support.trustwave.com (verified above), and in the 2026-08-27/28 batch alone
# weatherapi.com (a weather API vendor), sunshaholidays.com, gulfsqas.com and aartitrade.com.tr,
# every one a non-finance domain serving ticker-specific "analysis" pages. Blocking domains
# one-by-one loses that race; the path slugs are the stable fingerprint ("aticles-market" is
# the network's own typo of "articles" — no legitimate publisher uses it).
# Matched against the FULL lowercased URL, so a host+path fingerprint like the abcnyheter
# entry (search-redirect blobs, a results page is never a citable source) also fits.
LOW_TRUST_PATTERNS = ("/expert-time/", "/aticles-market/", "/first-dry/",
                      "startsiden.abcnyheter.no/sok/")
...
def drop_low_trust(urls):
    ...
    for u in urls:
        ...
        bad = (any(p in d for p in LOW_TRUST_DOMAINS)
               or any(p in u.lower() for p in LOW_TRUST_PATTERNS))
        (dropped if bad else kept).append(u)
    return kept, dropped
...
# LDR's formatted_findings source blocks: "[3, 7] Title (source nr: 3, 7)\n   URL: https://..."
# (format_links_to_markdown in local_deep_research/utilities/search_utilities.py). Those indices
# are the SAME numbering the synthesized summary's [N] citations refer to.
_CITE_MAP = re.compile(r"^\[([0-9, ]+)\][^\n]*\n\s+URL:\s+(\S+)", re.M)


def numbered_sources(res):
    """[(indices, url), ...] in first-seen order — the citation numbering the summary actually
    uses. Empty when LDR returned no mapping (then the caller falls back to bare URLs).

    WHY: the brief used to list extract_urls() output AFTER low-trust filtering and a [:15] cap,
    while the prose kept LDR's internal [N] numbers — so CIEN's moat section cited [6] above a
    2-URL source list and no [N] in any brief was resolvable, under a header saying 'Cite only
    the listed sources'. Emitting the mapping's own indices keeps every [N] resolvable and makes
    the filtering auditable instead of silent."""
    ff = res.get("formatted_findings") or ""
    # Prefer the deduplicated ALL SOURCES block; per-section source lists repeat entries.
    tail = ff.split("## ALL SOURCES:")[-1]
    out, seen = [], set()
    for m in _CITE_MAP.finditer(tail):
        idx, u = m.group(1).strip(), m.group(2).rstrip(".,)")
        if u not in seen:
            seen.add(u)
            out.append((idx, u))
    return out
...
def flag_consistency(t, sections):
    """FLAG-ONLY cross-section contradiction check (2026-08-29). Topics are researched
    independently, so the same quantity can land twice with incompatible values and no stage
    compared them: one GOOG brief carried 2026 capex as $85B in Catalysts and $195-205B in the
    capex section; a CIEN brief stated two different FY26 gross-margin ranges — and the analyst
    layer reads both as fact. Asks the already-loaded research model to list direct numeric
    contradictions, then logs + telegrams them. NEVER voids the brief and NEVER raises: a false
    positive here must cost a log line, not a ticker. The brief header separately instructs the
    analyst to treat conflicting numbers as unconfirmed."""
    if len(sections) < 2:
        return
    body = "\n\n".join(f"### {ti}\n{sm}" for ti, sm in sections)
    prompt = (
        "Below are research sections about one company, written independently of each other.\n"
        "List every pair of NUMERIC claims that directly contradict each other — between "
        "sections or within one section — the same quantity for the same period with "
        "incompatible values (e.g. two different full-year capex guidances). Ignore figures "
        "that differ because they cover different periods, segments or metrics, and ignore an "
        "actual result compared against a guidance, forecast or estimate for it — a beat or "
        "miss is not a contradiction. Ignore differences attributable to rounding "
        "(e.g. 19.9% vs 20%).\n"
        "Output one line per contradiction: <quantity>: <section A> says <X>; <section B> "
        "says <Y> (name the section twice when both claims sit in the same one). If there "
        "are none, output exactly: NONE\n\n" + body)
    try:
        req = urllib.request.Request(
            CONFIG["ollama_endpoint"],
            data=json.dumps({
                "model": CONFIG["research_model"], "stream": False,
                "options": {"num_ctx": int(CONFIG.get("research_ctx", 8192))},
                "messages": [{"role": "user", "content": prompt}],
            }).encode("utf-8"),
            headers={"Content-Type": "application/json"})
        with urllib.request.urlopen(req, timeout=180) as r:
            reply = (json.loads(r.read()).get("message") or {}).get("content", "")
        reply = re.sub(r"<think>.*?</think>", "", reply, flags=re.S).strip()
        # ROUNDING FILTER, in code because the model does not reliably honor the prompt's
        # rounding exclusion (GRDN: flagged $1.41478B vs $1.414B). For each "says X; says Y"
        # line, compare the first number after each "says": within 1% => rounding, drop.
        # Different units on the same value ($2,050M vs $2.05B) still slip through — rare,
        # and a flag-only false positive costs a log line.
        kept_lines = []
        for line in filter(None, (ln.strip() for ln in reply.splitlines())):
            vals = re.findall(r"says\s+[^0-9;]*([0-9][\d,]*\.?\d*)", line)
            if len(vals) >= 2:
                try:
                    a, b = (float(vals[0].replace(",", "")), float(vals[1].replace(",", "")))
                    if a and b and min(a, b) / max(a, b) > 0.99:
                        continue
                except ValueError:
                    pass
        ...
        if not reply or reply.upper().startswith("NONE"):
            print(f"[deep_research] {t} consistency: no numeric contradictions flagged", flush=True)
            return
        print(f"[deep_research] {t} ::CONSISTENCY:: {reply[:400]}", flush=True)
        ops.notify_telegram(
            f"[RS2 ops] research_consistency — {t}: numeric contradictions between brief "
            f"sections (brief still written; header tells the analyst to treat them as "
            f"unconfirmed):\n{reply[:800]}")
    except Exception as e:
        print(f"[deep_research] {t} consistency check skipped ({str(e)[:80]})", flush=True)


def fresh(path, days, ticker=None):
    try:
        ...
        if ticker:
            t = ticker.upper()
            sd_dir = Path(CONFIG.get("screener_data_dir", HERE.parent / "Stock Screener" / "Stock Screener" / "public" / "data"))
            if not sd_dir.exists():
                sd_dir = Path(r"C:\Users\riper\Downloads\Stock Screener\Stock Screener\public\data")
            # 1. Invalidate if a newer SEC filing landed after brief was written
            ttm_file = sd_dir / "fundamentals_ttm.json"
            if ttm_file.exists():
                try:
                    ttm_data = json.loads(ttm_file.read_text(encoding="utf-8"))
                    rec = ttm_data.get("tickers", {}).get(t)
                    if rec and rec.get("filed"):
                        filed_ts = datetime.strptime(rec["filed"], "%Y-%m-%d").timestamp()
                        if filed_ts > mtime:
                            print(f"[deep_research] {t}: new SEC filing on {rec['filed']} post-dates research cache — invalidating cache", flush=True)
                            return False
                except Exception:
                    pass
            # 2. Invalidate if a scheduled earnings report passed since brief was written
            fin_file = sd_dir / "financials" / f"{t}.json"
            if fin_file.exists():
                try:
                    fin_data = json.loads(fin_file.read_text(encoding="utf-8"))
                    ned = fin_data.get("Next_Earnings_Date")
                    if ned:
                        ned_clean = ned.split()[0].strip()
                        ned_ts = datetime.strptime(ned_clean, "%Y-%m-%d").timestamp()
                        if ned_ts > mtime and time.time() > ned_ts:
                            print(f"[deep_research] {t}: quarterly earnings reported on {ned_clean} post-dates research cache — invalidating cache", flush=True)
                            return False
                except Exception:
                    pass
        return True
...
def build(ticker, name, force=False):
    ...
    if not force and fresh(path, int(CONFIG["research_cache_days"]), ticker=ticker):
        ...
        # Source lines are '- https://...' (pre-2026-08-29) or '- [3, 7] https://...' (numbered).
        uncited = not re.search(r"^- (?:\[[0-9, ]+\] )?https?://", cached_txt, re.M)
    ...
    L = [f"# DEEP RESEARCH BRIEF — {name + ' ' if name else ''}({t})",
         ...
         f"(full-page reads + citations). Cite only the listed sources. Sections are researched "
         f"independently; where numeric claims conflict between sections, treat both as "
         f"unconfirmed.", ""]
    ...
    sections = []            # (title, summary) of committed topics, for the consistency flag
    for title, qt in topics:
        ...
        # When LDR's citation-number mapping is available it is the authoritative source list
        # (extract_urls' `sources` ordering is NOT the numbering the prose cites).
        numbered = numbered_sources(res)
        if numbered:
            urls = [u for _, u in numbered]
        ...
        L.append("\n**Sources:**")
        if numbered:
            # Emit with the prose's own citation indices so every [N] resolves. A dropped
            # low-trust source keeps its slot (numbered, URL withheld) — deleting the line
            # would silently renumber nothing and leave dangling [N]s again. No [:15] cap
            # here: capping is exactly what broke resolvability, and the count is already
            # bounded upstream by research_max_results.
            kept = set(urls)
            for idx, u in numbered:
                L.append(f"- [{idx}] {u}" if u in kept
                         else f"- [{idx}] (removed: low-trust source)")
        else:
            L += [f"- {u}" for u in urls[:15]]
        L.append("")
        sections.append((title, summ))
    ...
    flag_consistency(t, sections)   # flag-only; never blocks the write
